...
function Pro_func( $atts, $content = null ){
  ...
  //   if(((trim($a['frm1']) != "") || (trim($a['frm2']) != ""))){

		// ob_start();
		// do_shortcode('[cforms name="'.$a['frm1'].'"]');
		// $a['frm1'] = ob_get_contents();
		// ob_end_clean();

		// ob_start();
		// do_shortcode('[cforms name="'.$a['frm2'].'"]');
  //   	$a['frm2'] = ob_get_contents();
  //   	ob_end_clean();
  //   }
      ...
      return '
      	<hr/>
		<div class="span4">
		  <img style="width: 300px; border: 1px solid rgb(221, 221, 221); padding: 10px;" src="http://104.154.37.201/wp-content/uploads/2015/01/kitchen_adventurer_lemon_mini.jpg">
		</div>

		<div class="span6">
		  <h3 style="border-top: 1px solid rgb(221, 221, 221); border-bottom: 1px solid rgb(221, 221, 221); padding: 6px;">
		    El Lago <span style="font-weight: lighter ! important; font-size: 15px;">- Parcelacion el Retiro - Cali</span>
		  </h3>

		  <ul style="list-style: outside none none; margin-left: 0px; font-size: 15px; line-height: 15px;">
		    <li><strong>Ciudad:</strong> Cali</li> 
		    <li><strong>Habitaciones:</strong> 3,2,4</li> 
		    <li><strong>Tipo:</strong> Apartamentos</li> 
		    <li><strong>Área:</strong> Apartamentos</li> 
		    <li><strong>Precio:</strong> Apartamentos</li>    
		  </ul>
...
add_shortcode( 'pro', 'Pro_func' );



//[frm]
function Forms_func( $atts, $content = null ){
    ...
    $val1 = ((trim($a['frm1']) != "") && (trim($a['frm2']) != ""));
    $val2 = ((trim($a['frm1']) != "") && (trim($a['frm2']) == ""));
    $val3 = ((trim($a['frm1']) == "") && (trim($a['frm2']) != ""));
    $str = "";
...
add_shortcode( 'frm', 'Forms_func' );




//[MdoVsl]
function MdoVsl_func( $atts, $content = null ){
    ...
    if($a['tipo'] == 1){
      return '
      <div class="span12 MdoVsl1">
        <div class="span3">
          <img src="'.$a['img'].'" class="img-responsive">
        </div>
        <div class="span9">
          <h1 class="titulo aliniacion">'.$a['titulo'].'</h1>
          <p class="parrafos aliniacion">'.$content.'</p>
        </div>
      </div>';      
    }else{
      return '
      <div class="span12 MdoVsl2">
        <div class="span9">
          <h1 class="titulo aliniacion">'.$a['titulo'].'</h1>
          <p class="parrafos aliniacion">'.$content.'</p>
        </div>
...
add_shortcode( 'MdoVsl', 'MdoVsl_func' );


//[servicios]
function servicios_func( $atts ){
    ...
    return '
		<div id="divpri" class="col-lg-3 col-sm-4 col-xs-6 span3 col-centered"> 
			<div id="divsec">
				<a href="#" class="thumbnail view view-first"> 
					<img src="'.$a['img'].'"> 
				</a>
				<div class="miniRow">
				...
				<div class="tituloThumbDivBtn">
					<a href="'.$a['link'].'" class="btn btn-default btnVerMas" style="position: relative; top: -16px;">Ver Más</a> </div>
				</div>
			</div> 
		</div>';
  
}
add_shortcode( 'servicios', 'servicios_func' );
...
function Mapa_func( $atts ){
  ...
  class themeslug_walker_nav_menu_Sedes extends Walker_Nav_Menu {
	  ...
	  function start_lvl( &$output, $depth ) {
		  ...
		  // build html
		  $output .= "\n" . $indent . '<div id="opcion'.$this->currentParent.'" class="collapse ' . $class_names . '">' . "\n";
	  }
	  ...
	  // add main/sub classes to li's and links
	   function start_el( &$output, $item, $depth, $args ) {
		  ...
		  $this->currentParent = ($item->menu_item_parent == "0")?$item->ID:$item->menu_item_parent;
		  global $wp_query;
		  $indent = ( $depth > 0 ? str_repeat( "\t", $depth ) : '' ); // code indent
		  // depth dependent classes
		  $depth_classes = array(
		  ...
		  $attributes .= ! empty( $item->Coordenadas )        ? ' data-Coordenadas="'    . esc_attr( $item->Coordenadas        ) .'"' : '';
		  $attributes .= ! empty( $item->Url_imagen )        ? ' data-Url_imagen="'    . esc_attr( $item->Url_imagen        ) .'"' : '';
		  $attributes .= (! empty( $item->url ) && $depth > 0)       ? ' href="'   . esc_attr( $item->url        ) .'"' : '';
		  $attributes .= ' class="menu-link list-group-item ' . ( $depth > 0 ? 'sub-menu-link' : ' active' ) . '"';
		  $attributes .=  $depth > 0 ? '' : ' data-toggle="collapse" data-parent="#MainMenuMap" href="#opcion'.$this->currentParent.'"';
  ...
  $menuParameters = array(
	'menu' => 'Sedes', 
	'echo' => 0,
	'walker' => new themeslug_walker_nav_menu_Sedes, 
	'items_wrap' => '<div id="MainMenuMap"><div class="list-group panel">%3$s</div></div>'
  );
  
  $MenuMap = strip_tags(wp_nav_menu( $menuParameters ), '<div><a>' );  
  
  $Scripto = '<script type="text/javascript">
    var map;

    function MapsFunc(opt){
		map = new GMaps({
			el: \'#map\',
			lat: opt.lat,
			lng: opt.lng
    ...
    $(document).ready(function(){
    	MapsFunc({
    		"lat":3.4334938,
    		"lng":-76.5280784,
    		"html":\'<div class="overlay"><img src="http://orthoplan.com.co/wp-content/uploads/2014/11/La-Luna.jpg" style="width:200px;"><div style="font-size:12px; height:25px; line-height: 25px;">Orthoplan</div><div class="overlay_arrow above"></div></div>\'
    	});

    	$(".menu-link").click(function(e){
    		e.preventDefault();
    		var loc = $(this).attr("data-coordenadas");
    		var img = $(this).attr("data-url_imagen");
    		var tex = $(this).text();
			var tex2 = $(this).attr("title")||"";
    		loc = loc.split(",");
    		if(loc && img && tex){			
		    	MapsFunc({
		    		"lat":loc[0],
		    		"lng":loc[1],
"html":\'<div class="overlay"><img src="\'+img+\'" style="width:200px;"><div style="font-size:12px; height:25px; line-height: 25px;">\'+tex+\'</div><div style="font-size:12px; height:25px; line-height: 25px;">\'+tex2+\'</div><div class="overlay_arrow above"></div></div>\'
		    	});    		
    		}
				...
				<div class="span12">
					<div class="row">
					  <div class="span11">
						...
						<div class="span6" id="boton2" >
							<a href="'.site_url().'/convenios" id="nav-reflection"><img src="'.get_bloginfo("template_url").'/images/home/alianzas.png" class="img-responsive" alt="Responsive image"/><a/>
						</div>
						<div class="span6" id="boton3">
							<a href="'.site_url().'/noticias" id="nav-reflection"><img src="'.get_bloginfo("template_url").'/images/home/noticias.png" class="img-responsive" alt="Responsive image"/><a/>
						</div>
...
//dd_action( 'wp_enqueue_scripts', 'fruitful_scripts' );
